...
# NO SUDO REQUIRED - Uses conda/pip for all dependencies
#
# MODIFICATIONS MADE:
# - Removed all sudo apt-get commands
# - Uses conda-forge for mesa/OpenGL libraries
# - Uses pip for Python OpenGL bindings
# - Added OpenGL verification step
...
# Install system dependencies via conda/pip (no sudo required)
echo -e "${GREEN}[3/10] Installing system dependencies via conda/pip...${NC}"
echo "Installing OpenGL and graphics libraries via conda..."
...
echo -e "${GREEN}✓ System dependencies installed via conda/pip (no sudo required)${NC}"
