...
class BodyMocap(object):
    def __init__(self, regressor_checkpoint, smpl_dir, device=torch.device('cuda'), use_smplx=False, 
                 use_amp=False, n_iter=3):
        """
        Initialize BodyMocap model.
        
        Args:
            regressor_checkpoint: Path to model checkpoint
            smpl_dir: Directory containing SMPL model files
            device: torch.device (cuda or cpu)
            use_smplx: Use SMPLX model (True) or SMPL (False)
        ...
        self.use_amp = use_amp and self.device.type == 'cuda'  # Only enable AMP on CUDA
        self.n_iter = n_iter
    ...
    def regress(self, img_original, body_bbox_list, use_amp=None, n_iter=None):
        """
            args: 
                ...
                use_amp: Override AMP setting for this call (None = use instance default)
                n_iter: Override iteration count for this call (None = use instance default)
            outputs:
                pred_vertices_img:
                pred_joints_vis_img:
                pred_rotmat
        ...
        for body_bbox in body_bbox_list:
            ...
            with torch.no_grad():
                ...
                norm_img_tensor = norm_img.to(self.device)
                
                # Model forward with AMP if enabled
                ...
                # Optimize: keep on GPU longer, use device-aware operations
                if pred_rotmat.device.type == 'cuda':
                ...
                # Extract camera parameters (move to CPU only when needed)
                pred_camera_np = pred_camera.cpu().numpy().ravel()
                camScale = pred_camera_np[0] # *1.15
                camTrans = pred_camera_np[1:]
                
                # Move vertices to CPU for post-processing
                pred_vertices_np = pred_vertices[0].cpu().numpy()

                pred_output = dict()
                # Convert mesh to original image space (X,Y are aligned to image)
                # 1. SMPL -> 2D bbox
                # 2. 2D bbox -> original 2D image
                pred_vertices_bbox = convert_smpl_to_bbox(pred_vertices_np, camScale, camTrans)
                ...
                # Optimize: single CPU transfer
                pred_joints_3d_np = pred_joints_3d[0].cpu().numpy() # (1,49,3)
                pred_joints_vis = pred_joints_3d_np[:,:3]  # (49,3)
